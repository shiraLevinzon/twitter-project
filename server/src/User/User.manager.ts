...
export const updateRoll = async (req: Request): Promise<DocUserResponse> => {
  const { id }: { id?: string } = req.params;
  const userToUpdate : UserDocument = await userRepository.getUserById(id);
  const futureRole: Role  = userToUpdate.role === Role.manager ? Role.user : Role.manager;
  const updateUser: UserDocument = await userRepository.updateRoll(id, futureRole);
  return {
    status: 200,
    body: updateUser
  };
